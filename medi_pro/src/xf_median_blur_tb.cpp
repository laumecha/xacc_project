...
int main(int argc, char** argv) {
    ...
    cv::Mat in_img, out_img, final_img, ocv_ref, diff, in_mask;
...
//  Reading in the image:
    ...
    in_mask.create(in_img.rows, in_img.cols, CV_8UC1);
        ...
        size_t mask_in_size_bytes = in_mask.rows * in_mask.cols * sizeof(unsigned char);
...
// create memory for output image
    ...
    final_img.create(in_img.rows, in_img.cols, CV_8UC1);
...
// OpenCL section:
    ...
    /*NEW*/
    // Load binary:
        //std::string binaryFile2 = xcl::find_binary_file(device_name, "krnl_paintmask");
        //cl::Program::Binaries bins2 = xcl::import_binary_file(binaryFile2);
        devices.resize(1);
        //OCL_CHECK(err, program(context, devices, bins2, NULL, &err));

        // Create a kernel:
        OCL_CHECK(err, cl::Kernel kernel2(program, "paintmask_accel", &err));

        // Allocate the buffers:
        OCL_CHECK(err, cl::Buffer buffer_inImage_2(context, CL_MEM_READ_ONLY, image_in_size_bytes, NULL, &err));
        OCL_CHECK(err, cl::Buffer buffer_inMask_2(context, CL_MEM_READ_ONLY, image_in_size_bytes, NULL, &err));
        OCL_CHECK(err, cl::Buffer buffer_inColor_2(context, CL_MEM_READ_ONLY, image_in_size_bytes, NULL, &err));
        OCL_CHECK(err, cl::Buffer buffer_outImg_2(context, CL_MEM_WRITE_ONLY, image_out_size_bytes, NULL, &err));

        // Set kernel arguments:
        OCL_CHECK(err, err = kernel2.setArg(0, buffer_outImage));
        OCL_CHECK(err, err = kernel2.setArg(1, buffer_inMask_2));
        OCL_CHECK(err, err = kernel2.setArg(2, buffer_inColor_2));
        OCL_CHECK(err, err = kernel2.setArg(3, buffer_outImg_2));
        OCL_CHECK(err, err = kernel2.setArg(4, height));
        OCL_CHECK(err, err = kernel2.setArg(5, width));
        ...
        /*OCL_CHECK(err, queue.enqueueWriteBuffer(buffer_inImage,      // buffer on the FPGA
                                                ...
                                                0,                   // buffer offset in bytes
                                                image_in_size_bytes, // Size in bytes
												out_img.data,         // Pointer to the data to copy
                                                nullptr, &event));*/

        OCL_CHECK(err, queue.enqueueWriteBuffer(buffer_inMask_2,      // buffer on the FPGA
                                                CL_TRUE,            // blocking call
                                                0,                  // buffer offset in bytes
        ...
        OCL_CHECK(err, queue.enqueueWriteBuffer(buffer_inColor_2,    // buffer on the FPGA
                                                CL_TRUE,           // blocking call
                                                0,                 // buffer offset in bytes
        ...
        // Execute the kernel:
        OCL_CHECK(err, err = queue.enqueueTask(kernel2));

        // Copy Result from Device Global Memory to Host Local Memory
        queue.enqueueReadBuffer(buffer_outImg_2, // This buffers data will be read
                                CL_TRUE,       // blocking call
                                0,             // offset
                                image_out_size_bytes,
                                final_img.data, // Data will be stored here
                                nullptr, &event);
    ...
    cv::imwrite("hls_out.jpg", final_img); // kernel output
